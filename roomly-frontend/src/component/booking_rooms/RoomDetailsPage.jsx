...
const RoomDetailsPage = () => {
  const navigate = useNavigate(); // Access the navigate function to navigate(It returns a navigate function that allows to programmatically redirect or navigate the user to different routes in the app.)
  const { roomId } = useParams(); // Get room ID from URL parameters (It returns an object containing the parameters from the current URL.In this case, it's grabbing the value of a URL parameter called roomId from the route.)
  ...
  const handleConfirmBooking = async () => {
    ...
    if (isNaN(numAdults) || numAdults < 1 || isNaN(numChildren) || numChildren < 0) { //isNaN when the netered text is not a number
  ...
  const acceptBooking = async () => {
    try {
      ...
      const formattedCheckInDate = new Date(startDate.getTime() - (startDate.getTimezoneOffset() * 60000)).toISOString().split('T')[0]; //to UTC time 'Coordinated Universal Time'
